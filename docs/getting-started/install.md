...
```console
git clone https://github.com/simplexidev/codex-toolkit.git
cd codex-toolkit
...
On Windows, `--bin` is unsupported. Invoke `dotnet C:\path\to\codex-toolkit\tools\AgentTool.cs`
...
```console
codex plugin marketplace add /absolute/path/to/codex-toolkit
```
